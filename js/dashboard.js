...
function renderDashboard() {
  ...
  renderResumenEstados(data);
  renderCharts(data);
  renderFiltros('dashFilters');
  renderTablaSolicitudes('dashTable', data);
}

// ---------- RESUMEN POR ESTADO (respeta filtros activos) ----------
function renderResumenEstados(data) {
  const panel = document.getElementById('resumenEstados');
  if (!panel) return;
  ...
  data.forEach(s => {
    ...
    if (!porEstado[e]) porEstado[e] = { cantidad: 0, montoDOP: 0, impuesto: 0 };
    porEstado[e].cantidad++;
    porEstado[e].montoDOP += (s.montoDOP || 0);
    porEstado[e].impuesto += (s.impuestoExonerar || 0);
  });

  // Ordenar según el orden del catálogo de estados
  const entradas = ESTADOS
    .filter(e => porEstado[e])
    .map(e => [e, porEstado[e]]);

  // Estados que existen en datos pero ya no están en el catálogo
  Object.keys(porEstado).forEach(e => {
    if (!ESTADOS.includes(e)) entradas.push([e, porEstado[e]]);
  });
  ...
  const totCantidad = entradas.reduce((a, [, v]) => a + v.cantidad, 0);
  const totMonto = entradas.reduce((a, [, v]) => a + v.montoDOP, 0);
  const totImpuesto = entradas.reduce((a, [, v]) => a + v.impuesto, 0);

  const filas = entradas.map(([estado, v]) => `
    <tr>
      <td><span class="status-badge ${getStatusClass(estado)}"><span class="dot"></span>${escapeHtml(estado)}</span></td>
      <td class="mono" style="text-align:center">${v.cantidad}</td>
      <td class="money">${fmt(v.montoDOP)}</td>
      <td class="money" style="color:var(--accent);font-weight:600">${fmt(v.impuesto)}</td>
      <td class="mono" style="text-align:right;font-size:11px;color:var(--text-dim)">
        ${totImpuesto > 0 ? Math.round(v.impuesto / totImpuesto * 100) : 0}%
      </td>
    </tr>
  `).join('');

  const hayFiltros = filters.estado || filters.depto || filters.tipoCompra || filters.via || filters.mes || filters.search;

  panel.innerHTML = `
    <div class="card resumen-estados-card">
      <div class="card-header">
        <div class="card-title">
          Resumen por Estado
          <span class="card-count">${entradas.length}</span>
        </div>
        <span style="font-size:11px;color:var(--text-muted)">
          ${hayFiltros ? '⚙ Filtros activos aplicados' : 'Todas las solicitudes'}
        </span>
      </div>
      <div class="table-wrapper">
        <table>
          ...
          <tbody>
            ${filas}
            <tr class="totals-row">
              <td style="font-size:12px">TOTALES</td>
              <td class="mono" style="text-align:center">${totCantidad}</td>
              <td class="money">${fmt(totMonto)}</td>
              <td class="money">${fmt(totImpuesto)}</td>
              <td style="text-align:right" class="mono">100%</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    return `
      <div class="alert-row">
        ...
        <div class="alert-info">
          ...
          <div class="alert-sub">
            No. ${escapeHtml(numeroActual(a.sol) || '#' + a.sol.id)} · Vence: ${a.vencimiento} ·
