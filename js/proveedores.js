...
async function guardarProveedor() {
  ...
  await withLoading('Guardando proveedor...', async () => {
    if (editingProveedorId) {
      await api.proveedores.update(editingProveedorId, { nombre, rnc, contacto });
      ...
      if (p) {
        ...
        p.rnc = rnc;
        p.contacto = contacto;
        // Actualizar referencias en solicitudes (en memoria)
        if (oldNombre !== nombre) {
      ...
      const created = await api.proveedores.create({ nombre, rnc, contacto });
