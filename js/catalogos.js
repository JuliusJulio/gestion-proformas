...
function agregarItemCat(catKey) {
...
function renombrarItemCat(catKey, idx) {
  const arr = getCatArray(catKey);
  ...
  // Actualizar referencia de vigencia si aplica
  if (wasVigencia) {
    const vIdx = ESTADOS_VIGENCIA.indexOf(oldVal);
    if (vIdx >= 0) ESTADOS_VIGENCIA[vIdx] = trimmed;
  }

  // Actualizar todas las referencias en datos
  if (cat.refTable === 'contratos') {
    contratos.forEach(c => {
...
function eliminarItemCat(catKey, idx) {
  const arr = getCatArray(catKey);
  const val = arr[idx];
  const cat = getCatConfig(catKey);
  ...
  // Validación: si es el único estado disparador de vigencia, no permitir
...
function toggleVigenciaEstado(estado) {
