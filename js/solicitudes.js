...
function aplicarFiltros(data) {
  return data.filter(s => {
    ...
    if (filters.search) {
      ...
      const t = (s.proveedor + ' ' + s.descripcion + ' ' + numeroActual(s) + ' ' + numerosAnteriores(s).join(' ') + ' ' + (s.comentarios || '')).toLowerCase();
...
function renderTablaSolicitudes(containerId, data) {
  ...
  const rows = visibles.map(s => {
    ...
    return `
      <tr>
        <td class="num-cell">${escapeHtml(numeroActual(s) || '#' + s.id)}${numerosAnteriores(s).length > 0 ? ` <span style="cursor:pointer;color:var(--accent)" title="Este expediente tuvo otros números" onclick="event.stopPropagation();verHistorialNumeros(${s.id})">🕘</span>` : ''}</td>
  ...
  document.getElementById(containerId).innerHTML = `
    ...
    <div class="scroll-top-mirror" id="${containerId}_scrollTop">
      <div class="scroll-top-mirror-inner" id="${containerId}_scrollTopInner"></div>
    </div>
    <div class="table-wrapper" id="${containerId}_wrapper">
      <table>
        <thead>
          <tr>
            <th>No. Solicitud</th>
...
` : ''}
  ...
  sincronizarScrollSuperior(containerId);
}

// ---------- BARRA DE SCROLL ESPEJO (arriba de la tabla) ----------
function sincronizarScrollSuperior(containerId) {
  const top = document.getElementById(containerId + '_scrollTop');
  const inner = document.getElementById(containerId + '_scrollTopInner');
  ...
  const ajustar = () => {
    ...
    // Si no hay desbordamiento, la barra sobra
    top.style.display = wrapper.scrollWidth > wrapper.clientWidth ? '' : 'none';
  };

  ajustar();
  // Segunda pasada tras el layout (fuentes, anchos calculados)
  ...
  if (!window._scrollMirrorResize) {
    window._scrollMirrorResize = true;
    window.addEventListener('resize', () => {
      document.querySelectorAll('.scroll-top-mirror').forEach(el => {
        const cid = el.id.replace('_scrollTop', '');
        sincronizarScrollSuperior(cid);
      });
    });
  }
}
    ...
    <div class="form-section">
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <input type="text" id="fNoSol" value="${s ? escapeHtml(numeroActual(s) || '') : ''}" placeholder="Ej: 5270394">
          <div class="help-text">Si la solicitud cambia de número (re-sometimiento), regístralo en el evento correspondiente de la bitácora. El número actual será el del último evento.</div>
        </div>
        <div class="form-group">
...
function renderBitacora() {
  ...
  const rows = ordenada.map((b) => {
    ...
    return `
      <div class="bitacora-row" style="grid-template-columns: 1fr 110px 130px auto auto">
        <select onchange="cambiarBitacoraEstado(${b.originalIdx}, this.value)">
          ${estadosOpts}
        </select>
        <input type="text" value="${escapeHtml(b.numeroSolicitud || '')}" placeholder="No. solicitud"
               style="width:110px;font-family:var(--mono);font-size:11px"
               onchange="cambiarBitacoraNumero(${b.originalIdx}, this.value)"
               title="Número de solicitud vigente en este evento. Cambia al re-someter.">
  ...
  container.innerHTML = `
    <div class="bitacora">
      <div class="bitacora-header">
        <span>Estado</span>
        <span style="display:flex;gap:40px"><span>No. Solicitud</span><span>Fecha</span><span>Disparador</span></span>
      </div>
      ${rows}
...
function agregarEntradaBitacora() {
  // Pre-llenar con el número del evento más reciente (o el campo del formulario)
  let ultimoNumero = '';
  const ord = [...tempBitacora].sort((a, b) => (a.fecha || '').localeCompare(b.fecha || ''));
  for (let i = ord.length - 1; i >= 0; i--) {
    if (ord[i].numeroSolicitud) { ultimoNumero = ord[i].numeroSolicitud; break; }
  }
  if (!ultimoNumero) {
    const campo = document.getElementById('fNoSol');
    if (campo) ultimoNumero = campo.value.trim();
  }
  tempBitacora.push({
    estado: ESTADOS[0] || '',
    fecha: today(),
    disparaVigencia: false,
    numeroSolicitud: ultimoNumero,
  });
  renderBitacora();
}
...
function cambiarBitacoraNumero(idx, valor) {
  tempBitacora[idx].numeroSolicitud = valor.trim();
  renderBitacora();
}

// ---------- HISTORIAL DE NÚMEROS ----------
function verHistorialNumeros(solId) {
  const s = solicitudes.find(x => x.id === solId);
  if (!s) return;

  const filas = bitacoraOrdenada(s)
    .filter(e => e.numeroSolicitud)
    .map(e => `
      <tr>
        <td class="mono" style="font-size:12px">${escapeHtml(e.numeroSolicitud)}</td>
        <td><span class="status-badge ${getStatusClass(e.estado)}"><span class="dot"></span>${escapeHtml(e.estado)}</span></td>
        <td class="mono" style="font-size:12px">${e.fecha || '—'}</td>
      </tr>
    `).join('');

  document.getElementById('modalTitle').textContent = 'Historial de Números — ' + (numeroActual(s) || '#' + s.id);
  document.getElementById('modalBody').innerHTML = `
    <div style="font-size:13px;color:var(--text-muted);margin-bottom:12px">
      ${escapeHtml(s.proveedor)} — ${escapeHtml(s.descripcion)}
    </div>
    <table style="min-width:0;width:100%">
      <thead><tr><th>No. Solicitud</th><th>Estado del evento</th><th>Fecha</th></tr></thead>
      <tbody>${filas || '<tr><td colspan="3" style="text-align:center;color:var(--text-dim);padding:16px">Sin números registrados en la bitácora</td></tr>'}</tbody>
    </table>
    <div style="font-size:11px;color:var(--text-dim);margin-top:10px">
      Número actual: <strong class="mono">${escapeHtml(numeroActual(s) || '—')}</strong>
    </div>
  `;
  document.getElementById('modalFooter').innerHTML = `
    <div class="spacer"></div>
...
async function guardarSolicitud() {
  ...
  const proveedorId = proveedorIdByName(proveedor);
  if (!proveedorId) {
  ...
  await withLoading('Guardando solicitud...', async () => {
    if (editingSolicitudId) {
      await api.solicitudes.update(editingSolicitudId, solicitudData, proveedorId);
      ...
      const newId = await api.solicitudes.create(solicitudData, proveedorId);
      solicitudes.unshift({
        ...solicitudData,
        id: newId,
        proveedor_id: proveedorId,
